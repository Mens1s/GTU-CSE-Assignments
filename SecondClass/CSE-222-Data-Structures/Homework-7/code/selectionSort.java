...
/**
 * this class sorts map by using seleciton sort algorithm
 * */
public class selectionSort {
    ...
    /**
     * constructor of selection sort class it initialize originalMap and sortedMap variables
    ...
    public void sort(){
        ...
        for (int i = 0; i < n-1; i++)
        {
            int min_idx = i;
            for (int j = i+1; j < n; j++)
                if (sortedMap.getMap().get(aux[j]).getCount() < sortedMap.getMap().get(aux[min_idx]).getCount())
                    min_idx = j;
            Character temp = aux[min_idx];
            aux[min_idx] = aux[i];
            aux[i] = temp;
        }
        updateSortedMap();
